...
## 2024-06-24 - Precomputed Lookup Tables (LUT) for CRC Validation
**Learning:** Programmatic bitwise loops for CRC calculations (like O(N*8) loops in `calculateCRC`) consume significant CPU overhead in high-frequency validation paths (e.g., `receive()`).
