...
## 2026-05-26 - [Algorithmic Fast-Path for Unaddressed Packets]
**Learning:** Performing computationally expensive operations like `memcpy` deserialization and CRC validation on network packets that are destined for other devices wastes significant CPU cycles in high-frequency event loops.
**Action:** When using an algorithmic fast-path to skip unaddressed packets before parsing, always inspect the target ID directly from the raw incoming buffer (e.g., `buf[3]`). Ensure the `TALLY_BROADCAST_ID` is also verified so that valid broadcast packets are not incorrectly skipped.
