...
## 2026-05-25 - [Eliminate Redundant Hardware Transactions in SPI Drivers]
**Learning:** In SPI drivers, executing sequential blocking SPI queries for related states (e.g., calling `checkConnection()` immediately after `getChipStatus()`) creates redundant transactions that waste CPU cycles and stall the execution thread. Many secondary states can often be derived from primary query results.
**Action:** Always derive secondary connection states directly from the primary status byte returned by the initial query (e.g., `bool connected = (st != 0xFF && st != 0x00);`) instead of initiating new hardware queries.
...
## 2024-05-24 - Precomputed Lookup Table (LUT) for CRC calculation
**Learning:** In high-frequency validation logic like packet decoding, nested loop bitwise operations (like calculating CRC-8 byte by byte) introduce unnecessary O(N) operations per byte and CPU cycles overhead.
**Action:** Always replace programmatic bitwise loops in algorithms like CRC with precomputed static Lookup Tables (LUT). This trades a small amount of flash memory (e.g., 256 bytes) for a drastic O(1) performance boost in high-frequency validation logic. Ensure to maintain dynamic iteration bounds corresponding to actual data payload structure rather than manually unrolling the whole structure loop if it operates over system constants, to prevent silent regressions.
