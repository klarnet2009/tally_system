...
## 2026-06-15 - CRC-8 LUT Optimization
**Learning:** Programmatic bitwise loops for CRC calculation introduce significant overhead due to multiple branch instructions, especially when computed frequently (e.g., on every received/sent packet).
**Action:** Replace programmatic bitwise loops in algorithms like CRC with precomputed Lookup Tables (LUT) to trade a small amount of flash memory for a drastic O(1) performance boost in high-frequency validation logic.
