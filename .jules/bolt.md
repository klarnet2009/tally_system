...
## 2026-03-19 - [Early Return in SPI Polling Loops]
**Learning:** In high-frequency polling loops like `waitBusy()` for ESP32 SPI interfaces, avoiding unconditional calls to `millis()` and variable initialization saves measurable CPU cycles when the common case is 'not busy'.
**Action:** Add early return fast-paths like `if (digitalRead(pin) == LOW) return;` before setting up timeouts to avoid the overhead of `millis()` calls and variable initialization.
