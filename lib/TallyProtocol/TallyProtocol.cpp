...
// Precomputed Lookup Table for CRC-8/CCITT (poly 0x07)
...
uint8_t TallyProtocol::calculateCRC(const TallyPacket& packet) {
    ...
    // ⚡ Bolt: Using a precomputed Lookup Table (LUT) for O(1) byte processing,
    // replacing the slow inner bitwise loop to significantly speed up packet
    // validation which runs on every single received transmission.
