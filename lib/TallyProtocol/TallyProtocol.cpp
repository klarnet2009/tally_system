...
// ⚡ Bolt: Static Lookup Table (LUT) for CRC-8/CCITT (poly 0x07)
// Generated to replace the expensive bitwise inner loop in calculateCRC
...
uint8_t TallyProtocol::calculateCRC(const TallyPacket& packet) {
    ...
    // ⚡ Bolt: Fast O(1) table lookup replaces the inner bitwise O(N) loop
