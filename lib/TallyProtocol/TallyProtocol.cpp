...
uint8_t TallyProtocol::calculateCRC(const TallyPacket& packet) {
    ...
    // ⚡ Bolt: 256-byte Lookup Table (LUT) for CRC-8/CCITT (poly 0x07)
    // Trades a small amount of flash for a drastic O(1) performance boost
    // on high-frequency validation paths.
