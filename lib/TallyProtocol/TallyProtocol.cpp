...
// ⚡ Bolt: Static 256-byte Lookup Table (LUT) for CRC-8/CCITT (poly 0x07)
// to bypass expensive bitwise loop calculations on every packet serialization/validation
...
uint8_t TallyProtocol::calculateCRC(const TallyPacket& packet) {
    ...
    // ⚡ Bolt: Fast-path CRC calculation via static LUT array lookups
